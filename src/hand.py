    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                print("Previos: ", prev_cur_pos)
                print("Current: ", curr_cur_pos)
                for t in [x / 10.0 for x in range(1, 10, 1)]:
                    print(t, end=' ')
                    delta_cur_pos = curr_cur_pos.lerp(prev_cur_pos, t)
                    win32api.SetCursorPos(delta_cur_pos.getIntTup())
                    #print("Delta: ", delta_cur_pos)
